...
function normalizeAdvisorOutline(parsed, current, capacity, state = {}, event = {}) {
  ...
  return {
    ...
    policyIds: [...new Set((routes.length ? routes : dimensions.filter((item) => item.role !== '暂缓')).map((item) => item.policyId))],
  };
}
...
{
  "routes":[
    {"title":"路线名称","dilemmaTitle":"必须逐字使用当前困境名称","advice":"这条路线单独采纳后要做什么","tradeoff":"相对其他路线的成本、风险和放弃之处","policyId":"对应的一项政务ID"}
  ],
  "dimensions":[

  ...
  return (
    <div className="space-y-4">
      {history.map((entry, index) => {
        ...
        return (
          <article key={`${node.id}-${entry.option.label}`} className="border-l-2 border-cinnabar/55 bg-paper/45 p-4">
            ...
            <p className="mt-2 leading-7 text-brownInk">{entry.option.feedback}</p>
          </article>
        );
      })}
    </div>
